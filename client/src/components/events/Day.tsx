...
const Day = ({day_obj, date, events_update, user_events} : Props) => {
    ...
    return (
        ...
        {day_obj ? Object.keys(day_obj).map((hour, i) => (<Hour key={i} hour_obj={day_obj[hour]} time={hour} e_date={date} events_update={events_update} day = {day} user_events={user_events}/>)) : <h3>Loading</h3>}
        </div>
    )
}
